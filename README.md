...
This is the official JavaScript/TypeScript SDK for Cored server, used to interact with the Cored server via OpenAPI. You need to deploy the Cored server before using this SDK. See the [Quick Deploy Guide](https://coredim.com/docs/admin/install/quick-install) for setup instructions.
...
const resp = await client.Im.v1.Message.sendMessage({
  chat_id: 'chat-id',
  msg_type: 'text',
  content: JSON.stringify({ text: 'Cored new version released!' }),
});
console.log(resp);
